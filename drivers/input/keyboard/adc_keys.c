...
static int kp_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_err(&pdev->dev, "adc_key: faild to get key_num!\n");
		state =  -EINVAL;
		goto get_key_node_fail;
	}
	ret = of_property_read_u32(pdev->dev.of_node, "name_len", &name_len);
	if (ret) {
		dev_err(&pdev->dev, "adc_key: faild to get name_len!\n");
		name_len = 20;
	}
	...
	if (ret) {
		dev_err(&pdev->dev, "adc_key: faild to get key_code!\n");
		goto get_key_param_failed;
	}
	ret = of_property_read_u32_array(pdev->dev.of_node,
		 "key_chan", key_param+pdata->key_num, pdata->key_num);
	if (ret) {
		dev_err(&pdev->dev, "adc_key: faild to get key_chan!\n");
		goto get_key_param_failed;
	}
	ret = of_property_read_u32_array(pdev->dev.of_node,
		 "key_val", key_param+pdata->key_num*2, pdata->key_num);
	if (ret) {
		dev_err(&pdev->dev, "adc_key: faild to get key_val!\n");
		goto get_key_param_failed;
	}
	ret = of_property_read_u32_array(pdev->dev.of_node,
		 "key_tolerance", key_param+pdata->key_num*3, pdata->key_num);
	if (ret) {
		dev_err(&pdev->dev, "adc_key: faild to get tolerance!\n");
		goto get_key_param_failed;
	}
